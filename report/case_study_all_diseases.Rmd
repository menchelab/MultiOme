...
pacman::p_load(treemap, igraph, ggraph, tidygraph)
...
diseases <- rare_genetic_diseases_genes$disgene_df %>% pull(name)

save(list = c("g", "network_info", "processed_result_df", "rare_genetic_diseases_genes",  "diseases"  ), file = "../shiny/preloadeddata.RData")
```
...
```{r}
#visNetwork - website
disease_of_interest <- diseases[3]
graph_of_interest <- "HP"
...
nodes_df <- tibble(id = genes_set, label = genes_set)
edges_df <- igraph::as_data_frame(disease_graph)

  visNetwork(nodes = nodes_df, edges = edges_df, width = "100%") %>%
...
disease_of_interest <- diseases[2]

genes_set <- rare_genetic_diseases_genes$disgene_list[[disease_of_interest]]


signif_nets <- processed_result_df %>% 
