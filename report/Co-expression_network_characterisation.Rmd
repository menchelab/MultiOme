---
title: "coexpression network anlysis"
...
# bar plot
changelabel = function(x){gsub("\\(","", gsub("\\]", "", gsub(",","-", x)))} # change open-close values in bins


coex_el_sum$A.essential = coex_el_sum$A %in% essential_genes_sym
coex_el_sum$B.essential = coex_el_sum$B %in% essential_genes_sym
coex_el_sum = coex_el_sum %>% mutate(essential_edge_score = A.essential + B.essential)
coex_el_sum = coex_el_sum %>% mutate(n_binned = cut(n, breaks = c(seq(0,30,5), 38)))

coex_el_sum_grouped = coex_el_sum  %>% group_by(n_binned, essential_edge_score) %>% count()
...
# %>% mutate(
# score = factor(score, labels = c("none", "one", "both"), ordered = T )
#)


# create a vector of total probability for each class
coex_el_sum_score  = coex_el_sum_grouped %>% ungroup() %>% group_by(essential_edge_score) %>% summarise(count = sum(n)) %>% pull(count, name = essential_edge_score)
```

# Visualisation of the binned values
...
coex_bin_count = coex_el_sum_grouped %>% group_by(n_binned_relabel) 


EE_count <- coex_el_sum %>% ungroup() %>% count(n_binned, essential_edge_score)

coex_bin_count_plot = ggplot(coex_bin_count) + geom_bar(aes(x = n_binned_relabel, y = count/1e3), stat = "identity", alpha = 0.7) +
  xlab("") + ylab("edge count") + theme_cowplot() + scale_y_log10(breaks = c(1,10,100,1000,10000), labels = c("1k","10k","100k","1M", "10M")) 

coex_bin_count_plot = ggplot(coex_bin_count) + geom_bar(aes(x = n_binned_relabel, y = count), stat = "identity", alpha = 0.7) +
  xlab("") + ylab("edge count") + theme_cowplot() + scale_y_log10() 

ggsave("./analysis/network_characterisation/essentiality/edge_count_coex.pdf", coex_bin_count_plot, width = 6,height = 2)


#coex_bin_count_not_binned = coex_el_sum %>% ungroup %>% count(n)

coex_bin_count_plot = ggplot(coex_bin_count) + geom_bar(aes(x = n_binned_relabel, y = count), stat = "identity", alpha = 0.7) +
  xlab("") + ylab("edge count") + theme_cowplot() + scale_y_log10() 
ggsave("./analysis/network_characterisation/essentiality/edge_count_coex_tall.pdf", coex_bin_count_plot, width = 6,height = 3)
...
#=======================
# Nodes and edges for each group
