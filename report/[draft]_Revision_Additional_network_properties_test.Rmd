...
local_assortativity <- sapply(ppi_degree, function(x) igraph::assortativity(g$PPI_HIPPIE,types1 = x, directed = F))
local_assortativity <- igraph::assortativity(g$PPI_HIPPIE,types1 = degree(g$PPI_HIPPIE), directed = F)


ppi_degree_unique  = table(ppi_degree) 
#ppi_degree_unique = ppi_degree_unique[-1]

#p_k = ppi_degree_unique/sum(ppi_degree_unique)
...
cuts <- c(0,5, 10,20, 50,100, 200, 500,1000,5000)
...
ggplot(citation_assortativity, aes(x = degree, y = local_assortativity, col = (local_assortativity>0)))+ geom_point() + scale_x_log10() + theme_cowplot()  + guides(col = F) + xlab("Degree") + ylab("Local assortartivity") + ggrepel::geom_text_repel(aes(label = label))

p_density_assortativity <- 
