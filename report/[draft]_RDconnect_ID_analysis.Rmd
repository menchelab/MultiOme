...
solved_patient_annotated <- solved_patients %>% 
  ...
  rename(`Participant ID`="Patient", causal_gene="GeneName") %>%
  mutate(causal = TRUE)
...
if(!file.exists(patient_network_ranking_results)){
  source("../source/test/patient_rdconnect.R")
}
...
selected_features <- c("rank_HP", "rank_popularity", "rank_expression_BrainAll", "rank_pathwaycounts", "informedMultiPlex")
...
#pval_df <- combn(selected_features, 2) %>% t() %>% as_tibble() %>%
pval_df <- expand.grid(selected_features, selected_features) %>%
mutate(Var1 = factor(Var1, levels = selected_features, labels = selected_features_label, ordered = T),
      Var2 = factor(Var2, levels = selected_features, labels = selected_features_label, ordered = T)) %>%
  filter(Var1 >= Var2) %>%
  rowwise() %>%  
mutate(pval = pROC::roc.test(rocvals[[Var1]], rocvals[[Var2]])$p.value,
...
# palettes <-  c(RColorBrewer::brewer.pal(6, "Greens"), RColorBrewer::brewer.pal(3, "Blues"))
# 
# rank_group$network_set <- factor(rank_group$network_set, levels = c(unique(feature_based_rank$network_set), unique(network_based_rank$network_set)))
# 
# p <- ggplot(rank_group, aes(x = network_set, y = n, fill = network_set)) + 
...
## Number of patients found in each methods
...
Temporal holdout CV is to check how many 
...
AUC_plot_nonPanel <- gene_rank_data %>% ungroup %>% filter(`Experiment ID` %in% Patient_gene_not_in_Panel) %>%
