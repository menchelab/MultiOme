...
p <- ggplot(rocvals_merged_df_network, aes(x = Specificity, y = Sensitivity, col = rank_type)) + 
  geom_line(size=2) + 
  scale_x_reverse() + 
...
selected_features_label <- c("Feature: Phenotype overlap", "Feature: PubMed counts", "Feature: Brain expression level", "Feature: Pathway counts", "Informed Multiplex Propagation")
...
p <- ggplot(rank_group %>% filter(network_set %in% selected_features, group %in% c("(0,5]", "(5,10]", "(10,20]")) %>%
                                    mutate(network_set = factor(network_set, levels = selected_features, labels = selected_features_label)), aes(x = network_set, y = n, fill = network_set)) + 
 geom_col(position = "dodge") +
  facet_grid(.~ group) +
  scale_fill_manual(values = palettes) + #guides(col = FALSE) +
  cowplot::theme_cowplot() +
